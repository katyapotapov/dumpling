/*
        ...
        High-level API
                ...
                We create cs_playing_sound_t's indirectly with cs_play_def_t structs. cs_play_def_t
   is a POD struct so feel free to make them straight on the stack. The cs_play_def sets up
   initialization parameters. Here's an example to load a wav and play it:
        ...
        Low-level API
                ...
                parameters:
                        ...
                        frequency        --  int, represents Hz frequency rate in which samples are
   played buffered_samples --  int, number of samples the internal ring buffers can hold at once 0
   --  int, number of elements in cs_playing_sound_t pool NULL             --  optional pointer for
   custom allocator, just set to NULL
        ...
        KNOWN LIMITATIONS
                ...
                * PCM mono/stereo format is the only formats the LoadWAV function supports. I don't
                        guarantee it will work for all kinds of wav files, but it certainly does for
   the common kind (and can be changed fairly easily if someone wanted to extend it).
                * Only supports 16 bits per sample.
                * Mixer does not do any fancy clipping. The algorithm is to convert all 16 bit
   samples to float, mix all samples, and write back to audio API as 16 bit integers. In practice
   this works very well and clipping is not often a big problem.


        FAQ

                Q : Why DirectSound instead of (insert API here) on Windows?
                A : Casey Muratori documented DS on Handmade Hero, other APIs do not have such good
   docs. DS has shipped on Windows XP all the way through Windows 10 -- using this header
   effectively intro- duces zero dependencies for the foreseeable future. The DS API itself is sane
   enough to quickly implement needed features, and users won't hear the difference between various
   APIs. Latency is not that great with DS but it is shippable. Additionally, many other APIs will
   in the end speak to Windows through the DS API.

                Q : I would like to use my own memory management, how can I achieve this?
                A : This header makes a couple uses of malloc/free, and cs_malloc16/cs_free16.
   Simply find these bits and replace them with your own memory allocation routines. They can be
   wrapped up into a macro, or call your own functions directly -- it's up to you. Generally these
   functions allocate fairly large chunks of memory, and not very often (if at all).

                Q : Does this library support audio streaming? Something like System::createStream
   in FMOD. A : No. Typically music files aren't that large (in the megabytes). Compare this to a
   typical DXT texture of 1024x1024, at 0.5MB of memory. Now say an average music file for a game is
   three minutes long. Loading this file into memory and storing it as raw 16bit samples with two
   channels, would be:
                ...
                So say the user has 2gb of spare RAM. That means we could fit 67 different three
   minute length music files in there simultaneously. That is a ridiculous amount of spare memory.
   30mb is nothing nowadays. Just load your music file into memory all at once and then play it.

                Q : But I really need streaming of audio files from disk to save memory! Also
   loading my audio files (like .OGG) takes a long time (multiple seconds). A : It is recommended to
   either A) load up your music files before they are needed, perhaps during a loading screen, or B)
   stream in the entire audio into memory on another thread. cs_read_mem_ogg is a great candidate
   function to throw onto a job pool. Streaming is more a remnant of older machines (like in the
   90's or early 2000's) where decoding speed and RAM were real nasty bottlenecks. For more modern
   machines, these aren't really concerns, even with mobile devices. If even after reading this Q/A
   section you still want to stream your audio, you can try mini_al as an alternative:
...
// `CUTE_SOUND_FREE`.
cs_context_t* cs_make_context(void* hwnd, unsigned play_frequency_in_Hz, int buffered_samples,
                              int playing_pool_count, void* user_allocator_context);
...
int cs_insert_sound(
    cs_context_t* ctx,
    cs_playing_sound_t* sound);  // returns 1 if sound was successfully inserted, 0 otherwise
...
typedef struct cs_plugin_interface_t {
    ...
    /**
     * Called whenever a new sound is starting to play. `playing_sound_udata` is optional, and
     * should point to whatever data you would like to associate with playing sounds, on a
     * per-playing-sound basis.
     *
     * This function is only called from user threads whenever `cs_play_sound` or `cs_insert_sound`
     * is called.
     */
    void (*on_make_playing_sound_fn)(cs_context_t* cs_ctx, void* plugin_instance,
                                     void** playing_sound_udata, const cs_playing_sound_t* sound);

    /**
     * Called once for each time `on_make_playing_sound_fn` is called. The pointer originally
     * assigned in `on_make_playing_sound_fn` will be passed back here as `playing_sound_udata`. The
     * intent is to let the user free up any resources associated with the playing sound instance
     * before the sound is released completely internally.
     *
     * This function can be called from the user thread, or from the mixer thread.
     */
    void (*on_free_playing_sound_fn)(cs_context_t* cs_ctx, void* plugin_instance,
                                     void* playing_sound_udata, const cs_playing_sound_t* sound);

    /**
     * Called when mixing each playing sound instance, once per channel on each playing sound
     * instance. This function gives the plugin a chance to alter any audio before being mixed by
     * cute_sound to the audio driver. The source audio is not alterable, however, the plugin can
     * copy `samples_in` and then output modified samples in `samples_out`.
     *
     * `channel_index`       This function is called once per source channel, so `channel_index`
     * will be either 0 or 1, depending on the channel count. `samples_in`          All audio from
     * the playing sound's source. `sample_count`        The number of samples in `samples_in` and
     * expected in `samples_out`. `samples_out`         Required to point to a valid samples buffer.
     * The simplest case is to assign `samples_out` directly as `samples_in`, performing no
     * modifications. In most cases though the plugin will make a copy of `(float*)samples_in`,
     * alter the samples, and assign `samples_out` to point to the modified samples. `samples_out`
     * is expected to point to a valid buffer until the next time `on_mix_fn` is called. Typically
     * the plugin can hold a single buffer used for altered samples, and simply reuse the same
     * buffer each time `on_mix_fn` is called. `playing_sound_udata` The user data
     * `playing_sound_udata` assigned when `on_make_playing_sound_fn` was called.
     *
     * This function can be called from the user thread, or from the mixer thread.
     */
    void (*on_mix_fn)(cs_context_t* cs_ctx, void* plugin_instance, int channel_index,
                      const float* samples_in, int sample_count, float** samples_out,
                      void* playing_sound_udata, const cs_playing_sound_t* sound);
...
#endif
#define CUTE_SOUND_ASSERT(X)                  \
    do {                                      \
        if (!(X)) CUTE_SOUND_ASSERT_INTERNAL; \
    } while (0)
...
#pragma pack(pop)
    ...
    char* end = data + size;
    CUTE_SOUND_CHECK(
        data, "Unable to read input file (file doesn't exist, or could not allocate heap memory.");
    ...
    CUTE_SOUND_CHECK(fmt.nChannels == 1 || fmt.nChannels == 2,
                     "Only mono or stereo supported (too many channels detected).");
...
void cs_read_mem_ogg(const void* memory, int length, cs_loaded_sound_t* sound) {
    int16_t* samples = 0;
    int channel_count;
    int sample_count = stb_vorbis_decode_memory((const unsigned char*)memory, length,
                                                &channel_count, &sound->sample_rate, &samples);

    CUTE_SOUND_CHECK(
        sample_count > 0,
        "stb_vorbis_decode_memory failed. Make sure your file exists and is a valid OGG file.");
...
int cs_is_active(cs_playing_sound_t* sound) { return sound->active; }
...
void cs_sleep(int milliseconds) { Sleep(milliseconds); }
...
static void cs_release_context(cs_context_t* ctx) {
    ...
    while (playing) {
        for (int i = 0; i < ctx->plugin_count; ++i) {
            ...
            plugin->on_free_playing_sound_fn(ctx, plugin->plugin_instance, playing->plugin_udata[i],
                                             playing);
        }
...
static DWORD WINAPI cs_ctx_thread(LPVOID lpParameter) {
    ...
    while (ctx->running) {
        cs_mix(ctx);
        if (ctx->sleep_milliseconds)
            cs_sleep(ctx->sleep_milliseconds);
        else
            YieldProcessor();
    }
...
void cs_lock(cs_context_t* ctx) { EnterCriticalSection(&ctx->critical_section); }

void cs_unlock(cs_context_t* ctx) { LeaveCriticalSection(&ctx->critical_section); }

cs_context_t* cs_make_context(void* hwnd, unsigned play_frequency_in_Hz, int buffered_samples,
                              int playing_pool_count, void* user_allocator_ctx) {
    buffered_samples = buffered_samples < CUTE_SOUND_MINIMUM_BUFFERED_SAMPLES
    ...
    CUTE_SOUND_CHECK(hwnd, "Invalid hwnd passed to cs_make_context.");
    {
        ...
        ctx = (cs_context_t*)CUTE_SOUND_ALLOC(
            sizeof(cs_context_t) + mix_buffers_size + sample_buffer_size + 16 + pool_size,
            user_allocator_ctx);
...
void cs_lock(cs_context_t* ctx) { pthread_mutex_lock(&ctx->mutex); }

void cs_unlock(cs_context_t* ctx) { pthread_mutex_unlock(&ctx->mutex); }

static OSStatus cs_memcpy_to_coreaudio(void* udata, AudioUnitRenderActionFlags* ioActionFlags,
                                       const AudioTimeStamp* inTimeStamp, UInt32 inBusNumber,
                                       UInt32 inNumberFrames, AudioBufferList* ioData);

cs_context_t* cs_make_context(void* unused, unsigned play_frequency_in_Hz, int buffered_samples,
                              int playing_pool_count, void* user_allocator_ctx) {
    buffered_samples = buffered_samples < CUTE_SOUND_MINIMUM_BUFFERED_SAMPLES
                           ? CUTE_SOUND_MINIMUM_BUFFERED_SAMPLES
                           : buffered_samples;
    ...
    int mix_buffers_size = sizeof(cs__m128) * wide_count * 2;
    int sample_buffer_size = sizeof(cs__m128i) * wide_count;
    cs_context_t* ctx = (cs_context_t*)CUTE_SOUND_ALLOC(
        sizeof(cs_context_t) + mix_buffers_size + sample_buffer_size + 16 + pool_size,
        user_allocator_ctx);
    ...
    ret = AudioUnitSetProperty(inst, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0,
    ...
    ret = AudioUnitSetProperty(inst, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0,
                               &input, sizeof(input));
...
typedef struct cs_pcm_functions_t {
    ...
    int (*snd_pcm_hw_params_set_periods_first)(snd_pcm_t*, snd_pcm_hw_params_t*, unsigned int*,
                                               int*);
...
#define snd_pcm_sw_params_sizeof fns.snd_pcm_sw_params_sizeof
    ...
    CUTE_SOUND_CHECK(
        res >= 0,
        "Only interleaved sample output is supported by cute_sound, and is not available.");
    ...
    res = fns.snd_pcm_hw_params_set_rate_near(pcm_handle, hw_params,
                                              (unsigned*)&play_frequency_in_Hz, &dir);
    ...
    CUTE_SOUND_CHECK(res >= 0,
                     "Failed to set buffer size, likely not enough RAM for these settings.");
    res = fns.snd_pcm_hw_params_set_periods_min(pcm_handle, hw_params, &periods, &dir);
    CUTE_SOUND_CHECK(
        res >= 0,
        "Failed to set double buffering (min), likely not enough RAM for these settings.");
    res = fns.snd_pcm_hw_params_set_periods_first(pcm_handle, hw_params, &periods, &dir);
    CUTE_SOUND_CHECK(
        res >= 0,
        "Failed to set double buffering (first), likely not enough RAM for these settings.");
...
void cs_sleep(int milliseconds) { SDL_Delay(milliseconds); }
...
void cs_lock(cs_context_t* ctx) { SDL_LockMutex(ctx->mutex); }

void cs_unlock(cs_context_t* ctx) { SDL_UnlockMutex(ctx->mutex); }
...
cs_playing_sound_t* cs_get_playing(cs_context_t* ctx) { return ctx->playing; }

#if CUTE_SOUND_PLATFORM == CUTE_SOUND_SDL || CUTE_SOUND_PLATFORM == CUTE_SOUND_APPLE

static int cs_samples_written(cs_context_t* ctx) { return ctx->samples_in_circular_buffer; }
...
static void cs_push_bytes(cs_context_t* ctx, void* data, int size) {
    ...
    if (samples_to_write > samples_to_end) {
        memcpy((char*)ctx->samples + CUTE_SOUND_SAMPLES_TO_BYTES(index1), data,
               CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end));
        memcpy(ctx->samples, (char*)data + CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end),
               size - CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end));
...
static int cs_pull_bytes(cs_context_t* ctx, void* dst, int size) {
    ...
    if (samples_to_read > samples_to_end) {
        memcpy(dst, ((char*)ctx->samples) + CUTE_SOUND_SAMPLES_TO_BYTES(index0),
               CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end));
        memcpy(((char*)dst) + CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end), ctx->samples,
               size - CUTE_SOUND_SAMPLES_TO_BYTES(samples_to_end));
...
static void cs_memcpy_to_directsound(cs_context_t* ctx, int16_t* samples, int byte_to_lock,
                                     int bytes_to_write) {
...
#ifdef __cplusplus
    HRESULT hr =
        ctx->buffer->Lock(byte_to_lock, bytes_to_write, &region1, &size1, &region2, &size2, 0);
...
#else
    HRESULT hr = ctx->buffer->lpVtbl->Lock(ctx->buffer, byte_to_lock, bytes_to_write, &region1,
                                           &size1, &region2, &size2, 0);

    if (hr == DSERR_BUFFERLOST) {
        ctx->buffer->lpVtbl->Restore(ctx->buffer);
        hr = ctx->buffer->lpVtbl->Lock(ctx->buffer, byte_to_lock, bytes_to_write, &region1, &size1,
                                       &region2, &size2, 0);
    }
#endif
...
void cs_mix(cs_context_t* ctx) {
    ...
    while (*ptr) {
        ...
        if (!playing->active || !ctx->running) goto remove;

        if (!loaded) goto remove;
...

        {
            ...
            for (int i = 0; i < ctx->plugin_count; ++i) {
                ...
                plugin->on_mix_fn(ctx, plugin->plugin_instance, 0, plugin_samples_in_channel_a,
                                  sample_count, &samples_out_channel_a, playing->plugin_udata[i],
                                  playing);
                if (loaded->channel_count == 2)
                    plugin->on_mix_fn(ctx, plugin->plugin_instance, 1, plugin_samples_in_channel_b,
                                      sample_count, &samples_out_channel_b,
                                      playing->plugin_udata[i], playing);
                if (samples_out_channel_a) cA = (cs__m128*)samples_out_channel_a;
                if (samples_out_channel_b) cB = (cs__m128*)samples_out_channel_b;

                // Set offset_wide to cancel out delay_wide because cA and cB are now owned by the
                // plugin, this elimineating the need for the delay offset.
...
#elif CUTE_SOUND_PLATFORM == CUTE_SOUND_APPLE || CUTE_SOUND_PLATFORM == CUTE_SOUND_SDL || \
    CUTE_SOUND_PLATFORM == CUTE_SOUND_LINUX
